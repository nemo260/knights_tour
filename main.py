    ...
    if options:
        ...
        for i in options:
            if len(get_possible_steps(i[0], i[1])) <= len(get_possible_steps(actual_step[0], actual_step[1])):
                actual_step = i
        counter += 1
        tour_value = lets_tour_with_heuristic(actual_step[0], actual_step[1], counter, board_size, chess_board)
        if tour_value:
            return True
        else:
            counter -= 1
...
for i in range(1):
    ...
    print(lets_tour_with_heuristic(x, y, counter, board_size, chess_board))
    end = time.time()
